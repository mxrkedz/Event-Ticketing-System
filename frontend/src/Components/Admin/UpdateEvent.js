...
const UpdateEvent = () => {
    ...
    const getEventDetails =  async (id) => {
        try {
           ...
           const { data } = await axios.get(`${process.env.REACT_APP_API}/api/v1/event/${id}`, config)
           setEvent(data.event)
           setLoading(false)
    ...
    const updateEvent = async (id, eventData)  => {
        try {
            ...
            const { data } = await axios.put(`http://localhost:4001/api/v1/admin/event/${id}`, eventData, config)
