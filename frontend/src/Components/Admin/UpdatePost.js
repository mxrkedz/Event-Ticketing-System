...
const UpdatePost = () => {
    ...
    const getPostDetails =  async (id) => {
        try {
           ...
           const { data } = await axios.get(`${process.env.REACT_APP_API}/api/v1/post/${id}`, config)
           setPost(data.post)
           setLoading(false)
    ...
    const updatePost = async (id, postData)  => {
        try {
            ...
            const { data } = await axios.put(`http://localhost:4001/api/v1/admin/post/${id}`, postData, config)
